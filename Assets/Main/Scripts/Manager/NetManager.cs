...
public class NetManager : MonoSingleton<NetManager>
{
    ...
    public void AddRollingMSG(params string[] _args)
    {
        string[] _alertMsg = new string[3];
        _alertMsg[0] = _args[0];
        _alertMsg[1] = _args[1];
        _alertMsg[2] = _args.Length > 2 ? _args[2] : "3";
        rollingTextQue.Enqueue(_alertMsg);
    }
    ...
    public System.Collections.IEnumerator RollIngMSG()
    {
        while (true)
        {
            ...
            if (rollingTextQue.Count > 0)
            {
                ...
                float moveTime = float.Parse(_alertMsg[2]);
